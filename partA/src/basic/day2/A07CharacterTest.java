...
public class A07CharacterTest {
	...
	public static void main(String[] args) {
		...
		System.out.println("한글 유니코드 문자는 총 " + (code6-(int)c3+1) + "개 입니다.");
		
		// 시작값~마지막값이 주어지면 이 범위의 해당되는 개수를 구하는 수식?  마지막값 - 시작값 + 1
		// 예시 : 14부터 20 => 20-14+1 = 7개
		
		
		System.out.println("///int와 char형식 사이의 캐스팅///");
		int code3 = c3;			//c3은 '가' 를 int 로 자동 캐스팅하면 44032
		char temp = (char)code3;  //int를 char로 변경은 강제 캐스팅
		System.out.println("code3 : " + code3);		//44033
		System.out.println("문자로 캐스팅후 출력한 code3 : " + (char)code3);		//int를 char로 강제 캐스팅. '각'
		System.out.println("temp 변수 문자 : " + temp);		//int를 char로 강제 캐스팅. '각'
		...
		System.out.println("변경된 c3 문자의 코드값 : " + (int)c3);		//44033
